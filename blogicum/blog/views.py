...
from .mixins import (
    BasePostMixin, CommentBaseMixin, CommentObjectMixin, OwnerRequiredMixin
)
...
class PostDetailView(BasePostMixin, DetailView):
    ...
    def get_object(self, queryset=None):
        ...
        if not post.is_published and self.request.user != post.author:
            raise Http404("Post is unpublished or not accessible.")
        return post
...
class ProfileView(BasePostMixin, ListView):
    ...
    def get_queryset(self):
        ...
        post_list = self.user.posts.select_related(
            'category', 'location'
        ).annotate(
